...
class Note extends FlxSprite
{
	...
	public function new(strumLine:StrumLine, noteData:ChartNote, sustain:Bool = false, sustainLength:Float = 0, sustainOffset:Float = 0, ?prev:Note)
	{
		...
		var event = EventManager.get(NoteCreationEvent).recycle(this, strumID, this.noteType, noteTypeID, PlayState.instance.strumLines.members.indexOf(strumLine), mustPress,
			(this.noteType != null && customTypePathExists(customType)) ? 'game/notes/${this.noteType}' : 'game/notes/default/notes', @:privateAccess strumLine.strumScale * Flags.DEFAULT_NOTE_SCALE, animSuffix);
		...
		if (!event.cancelled) {
			switch (event.noteType)
			{
				...
				default:
					...
					rgbShader = new RGBShaderReference(this, initializeGlobalRGBShader(this.noteData));

					var strumID:Int = event.strumID % 4;
					var dirArray:Array<String> = ["left0", "down0", "up0", "right0"];

					animation.addByPrefix('scroll', dirArray[strumID]);
			...
			if (isSustainNote) 
			{
				frames = Paths.getFrames(FlxStringUtil.remove(event.noteSprite, "default/notes") + "default/hold");
				
				animation.addByPrefix('hold', 'hold part');
